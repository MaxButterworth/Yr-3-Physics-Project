...
for i in range(80, 310, 10):
    data = pd.read_csv('GaAs_{}K_Full_Data.csv'.format(i)) # Open the spectrum csv file
    ...
    ax1.plot(wavelength, intensity, label='Spectrum', linewidth=3)
    ax1.set_xlim(825, 1000)
    ...
    ax1.plot(wavelength, ((grad * wavelength) + y_int), label='Linear Fit', linewidth=3)
    
    # Displaying the overall plot
    ax1.set_ylim((0, 0.075e6))
    #plt.legend(loc='upper left')
    plt.tight_layout()
    plt.savefig('Spectrum and Linear Plot for {} K (Repeat 00).png'.format(i), dpi=300, edgecolor='auto')
    plt.show()
